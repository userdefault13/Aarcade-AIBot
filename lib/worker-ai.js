...
const DEPARTMENTS = [
  ...
  { id: 'creative', title: 'Creative', path: '/adminbusinessdevelopment/creative', description: 'Design, content creation, and creative operations. Kanban board for creative tasks.', keywords: ['creative', 'design', 'content', 'creative operations', 'visual', 'brand'], category: 'business' },
...
View your gotchis at /myaavegotchis, stake GHST at /stake, and trade on the Baazaar at /baazaar.`,
  ...
  'how-many-aavegotchis': {
    triggers: ['how many aavegotchi', 'how many gotchis', 'how many aavegotchis exist', 'total aavegotchis', 'aavegotchi supply', 'how many gotchis exist', 'number of aavegotchis'],
    content: `**23,040 Aavegotchis** currently exist.

Aavegotchis are ERC-721 NFTs minted on Polygon through **Portals**, where each opened Portal summons 10 unique Aavegotchis.

Two minting events (Haunts) produced 25,000 Portals total: **Haunt 1** (10,000 Portals) and **Haunt 2** (15,000 Portals). Not all have been opened, so the total minted Aavegotchis is lower.

Current stats from NFT trackers confirm exactly **23,040 Aavegotchis** minted and held by **5,647 unique owners** (main collection: Aavegotchi Official on Polygon, contract \`0x86935F11C86623deC8a25696E1C19a8659CbF95d\`).

This number is fixed unless more Portals are opened (requires GHST) or future Haunts are DAO-approved (none since 2021).`,
  },
  paarcel: {
...
export { DEPARTMENTS, AAVEGOTCHI_TOPICS, AARCADE_TOPICS, ECO_FINANCE_TOPICS, CRYPTO_TRADING_TOPICS, ALL_TOPICS, RICH_RESPONSES };

export function getReply(query, options = {}) {
  const customTools = options.customTools || [];
  const customRich = customTools.filter((t) => t.triggers?.length && t.content);
  const customTopics = customTools.filter((t) => t.keywords?.length && t.description).map((t) => ({
    id: t.id,
    title: t.title || t.id,
  ...
  for (const config of customRich) {
    if (config.triggers.some((t) => q.includes(String(t).toLowerCase()))) {
      return config.content;
    }
  }
  ...
  if (isList) {
    const allTopicsForList = [...ALL_TOPICS, ...customTopics];
    const byCategory = {};
    for (const t of allTopicsForList) {
      const cat = t.category || 'other';
      if (!byCategory[cat]) byCategory[cat] = [];
      byCategory[cat].push(t);
    }
    const catNames = { aavegotchi: 'Aavegotchi Ecosystem', aarcade: 'Aarcade Gh$t', business: 'Business (Admin)', finance: 'Finance & Economics', economics: 'Economics', trading: 'Crypto & Trading', custom: 'Custom' };
  ...
  const allTopicsForMatch = [...ALL_TOPICS, ...customTopics];
  const matched = [];
  for (const t of allTopicsForMatch) {
    const titleLower = (t.title || '').toLowerCase();
    const match = t.keywords.some((k) => q.includes(String(k).toLowerCase())) ||
      q.includes(t.id) ||
